...
public class UserInputController {
    ...
    @PostMapping
    @Operation(description = "User post service", responses = {
            @ApiResponse(responseCode = "201", ref = "badRequestAPI"),
            @ApiResponse(responseCode = "201", description = "Successfully created user!", content = @Content(mediaType = "application/json", examples = {
                    @ExampleObject(value = "{\"code\" : 201, \"Status\" : \"Created!\", \"Message\" :\"Successfully created user!\"}"),
            }))
    })
    public ResponseEntity<WebResponse> createUserData(@RequestBody UserData userData) {

        userDataService.createUserInput(userData);
        return new ResponseEntity<>(new WebResponse(HttpStatus.CREATED.value(), HttpStatus.CREATED.getReasonPhrase(),
                "Successfully created user!"), HttpStatus.CREATED);
